...
export function getBand({ channel, fieldDef, fieldDef2, markDef: mark, stack, config, isMidPoint }) {
    ...
    if (contains(['x', 'y', 'theta', 'radius'], channel)) {
        ...
        else if ((stack === null || stack === void 0 ? void 0 : stack.fieldChannel) === channel && isMidPoint) {
            return 0.5;
        }
    }
    return undefined;
...
export function isAnyPositionFieldOrDatumDef(channelDef) {
 ...
 */
export function initChannelDef(channelDef, channel) {
...
export function initFieldOrDatumDef(fd, channel) {
    if (isFieldDef(fd)) {
...
export function isFieldDefWithCustomTimeFormat(fieldOrDatumDef) {
    const guide = getGuide(fieldOrDatumDef);
    const formatType = (guide && guide.formatType) || (isTextFieldDef(fieldOrDatumDef) && fieldOrDatumDef.formatType);
