import { COLOR, FILL, FILLOPACITY, OPACITY, SHAPE, SIZE, STROKE, STROKEDASH, STROKEOPACITY, STROKEWIDTH } from '../../channel';
...
function parseUnitLegend(model) {
    ...
    return [COLOR, FILL, STROKE, STROKEWIDTH, STROKEDASH, SIZE, SHAPE, OPACITY, FILLOPACITY, STROKEOPACITY].reduce((legendComponent, channel) => {
...
export function parseLegendForChannel(model, channel) {
    ...
    const legendEncode = ['labels', 'legend', 'title', 'symbols', 'gradient', 'entries'].reduce((e, part) => {
        var _a;
        const legendEncodingPart = guideEncodeEntry((_a = legendEncoding[part]) !== null && _a !== void 0 ? _a : {}, model);
        const fieldOrDatumDef = getFieldOrDatumDef(model.encoding[channel]);
        const value = encode[part]
            ? encode[part](fieldOrDatumDef, legendEncodingPart, model, channel, legendCmpt) // apply rule
            : legendEncodingPart; // no rule -- just default values
        if (value !== undefined && keys(value).length > 0) {
            e[part] = Object.assign(Object.assign(Object.assign({}, ((selections === null || selections === void 0 ? void 0 : selections.length) && isFieldDef(fieldOrDatumDef)
